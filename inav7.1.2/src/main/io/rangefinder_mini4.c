...
static void mini4RangefinderInit(void)
{
    ...
    serialPort =
        openSerialPort(portConfig->identifier, FUNCTION_RANGEFINDER, NULL, NULL,
                       9600, MODE_RXTX, SERIAL_NOT_INVERTED);
    if (!serialPort)
    {
        return;
    }
...
static void mini4RangefinderUpdate(void)
{
    uint8_t checksum = 0;
    uint8_t flag = 0;
    if (lastProtocolActivityMs == 0 ||
        (millis() - lastProtocolActivityMs) > MINI4_TIMEOUT_MS)
    {
