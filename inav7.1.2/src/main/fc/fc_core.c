...
//    pinioBoxUpdate();
    ...
    if (IS_RC_MODE_ACTIVE(BOXUSER1)) {
        IOHi(IOGetByTag(IO_TAG(PINIO1_PIN)));
    } else {
        IOLo(IOGetByTag(IO_TAG(PINIO1_PIN)));
    }
    if (IS_RC_MODE_ACTIVE(BOXUSER2)) {
        IOHi(IOGetByTag(IO_TAG(PINIO2_PIN)));
    } else {
        IOLo(IOGetByTag(IO_TAG(PINIO2_PIN)));
    }
    if (IS_RC_MODE_ACTIVE(BOXUSER3)) {
        IOHi(IOGetByTag(IO_TAG(PINIO3_PIN)));
    } else {
        IOLo(IOGetByTag(IO_TAG(PINIO3_PIN)));
    }
    if (IS_RC_MODE_ACTIVE(BOXUSER4)) {
        IOHi(IOGetByTag(IO_TAG(PINIO4_PIN)));
    } else {
        IOLo(IOGetByTag(IO_TAG(PINIO4_PIN)));
    }
        if (!cliMode) {
            bool canUseRxData =
            ...
            processRcAdjustments(
                CONST_CAST(controlRateConfig_t *, currentControlRateProfile),
                canUseRxData);
        }
